	...
	public void notify(BindingsResults notify) {
		...
		if (notify.getAddedBindings() != null){
			for (Bindings results : notify.getAddedBindings()){
				...
				System.out.printf(" --\n");
				map.addMapping(network, new IoTContextAction(context,action,value),requestPattern, responsePattern);
			}
		}
		System.out.println("-------------------------------------------------------");
		...
		if (notify.getRemovedBindings() != null){
			for (Bindings results : notify.getRemovedBindings()){
				...
				System.out.printf(" --\n");
				map.removeMapping(network, new IoTContextAction(context,action,value),requestPattern, responsePattern);
			}
		}
		System.out.println("-------------------------------------------------------");
	}
